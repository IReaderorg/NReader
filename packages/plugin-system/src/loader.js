...
export class PluginLoader {
    ...
    async reloadPlugin(filename) {
        ...
        try {
            ...
            if (isIReaderSource(plugin)) {
                // Wrap IReader source through the bridge adapter
                const deps = createJsDependencies(plugin.baseUrl || `https://${pluginId}.local`);
                const adapter = createIReaderAdapter(plugin, deps);
