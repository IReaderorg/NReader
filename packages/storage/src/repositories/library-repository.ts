...
export class SqliteLibraryRepository implements LibraryRepository {
  ...
  async getAll(): Promise<LibraryEntry[]> {
      ...
      return rows.map(row => ({
        ...
        favorited: row.favorited ? Boolean(Number(row.favorited)) : undefined,
        archived: row.archived ? Boolean(Number(row.archived)) : undefined,
  ...
  async add(entry: LibraryEntry): Promise<void> {
      await this.driver.execute(
        `INSERT OR REPLACE INTO library (id, source_id, manga_id, title, cover_url, author, status, rating, genres, description, last_read_at, chapters_read, total_chapters, score, favorited, archived, date_added, date_updated, category_ids)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          entry.id,
          entry.sourceId,
  ...
  async update(entry: Partial<LibraryEntry> & { id: string }): Promise<void> {
      ...
      if (entry.favorited !== undefined) { fields.push('favorited = ?'); values.push(entry.favorited ? 1 : 0) }
      if (entry.archived !== undefined) { fields.push('archived = ?'); values.push(entry.archived ? 1 : 0) }
