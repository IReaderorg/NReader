...
export class SqliteSettingsRepository implements SettingsRepository {
  ...
  async getAll(): Promise<Setting[]> {
    const rows = await this.driver.query<{ key: string; value: string }>('SELECT * FROM settings')
    return rows.map(row => ({ key: row.key, value: JSON.parse(row.value) }))
  }

  async get(key: string): Promise<Setting | null> {
    const row = await this.driver.queryOne<{ key: string; value: string }>('SELECT * FROM settings WHERE key = ?', [key])
    if (!row) return null
